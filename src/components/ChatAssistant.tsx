...
export default function ChatAssistant() {
  ...
  const speak = (text: string) => {
    ...
    if (targetVoice && (targetVoice.name.includes('Yunxi') || targetVoice.name.includes('Yunjian') || targetVoice.name.includes('Yunze'))) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const assistantContent = data.answer || '抱歉，我暂时无法回答这个问题。';
      ...
      speak(assistantContent); // 自动语音播报回复
          ...
          <div className="w-10 h-10 rounded-full bg-white overflow-hidden flex items-center justify-center border-2 border-red-200/50 shadow-inner">
            <img 
              ...
              onError={(e) => {
                ...
                e.currentTarget.parentElement!.innerHTML = '<i class="fas fa-robot text-red-600 text-xl"></i>';
              }}
            />
          </div>
          <div>
            <h3 className="font-semibold text-lg tracking-wide">科学家知识助手</h3>
            <p className="text-xs text-red-100 opacity-90 font-light">基于百度 AppBuilder 驱动</p>
          </div>
        </div>
        <button 
          onClick={() => {
              ...
              {msg.role === 'assistant' ? (
                <div className="w-9 h-9 rounded-full bg-white overflow-hidden flex items-center justify-center border border-gray-200 shadow-sm">
                  <img 
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).style.display = 'none';
                      e.currentTarget.parentElement!.innerHTML = '<i class="fas fa-robot text-red-600 text-sm"></i>';
                    }}
                  />
                </div>
              ) : (
      ...
      <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800 z-10">
        <div className="flex items-end gap-3 max-w-4xl mx-auto">
          <button
            ...
            title={isRecording ? "停止录音" : isTranscribing ? "正在识别..." : "开始语音输入"}
          >
            {isTranscribing ? (
          ...
          <div className="flex-1 relative bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 focus-within:border-red-400 focus-within:ring-2 focus-within:ring-red-100 dark:focus-within:ring-red-900/30 transition-all">
            <textarea
              ...
              placeholder={isRecording ? "正在聆听，请说话..." : isTranscribing ? "正在识别语音..." : "输入您的问题，或点击左侧麦克风说话..."}
